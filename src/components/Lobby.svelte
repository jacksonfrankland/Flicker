...
<a class="text-coolGray-400 hover:underline text-lg ml-auto pr-2" href={'javascript:void(0)'} on:click={() => {dispatch('newGame')}}> New Game </a>
<div class="card grid max-w-2xl bg-white gap-8 my-auto">
    ...
    <ul>
        {#each $players.filter(player => player.name.length > 0).sort((a, b) => a.id - b.id) as player (player.id)}
            <!-- remember purge -->
            <li class="card shadow-none bg-{player.color}-300 text-center mb-2 p-2">
